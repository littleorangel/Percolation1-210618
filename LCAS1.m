...
while BondNew_ALL < Par2
    ...
    while BondNewNum < Par1 %确保增加Par1条链路后才计算一遍新阈值
        ...
        Clos = centrality(G1,'closeness'); %Clos：网络中节点的中间中心性向量
        ClosLoc = [1:1:Node_num]'; %ClosLoc：网络中节点的中间中心性所对应的位置向量
        ClosMin = min(Clos); %ClosMin：最小中间中心性
        ClosMinLoc = find(Clos == ClosMin); %ClosMinLoc：最小中间中心性对应的位置向量        
        ClosLC = [ClosLoc, Clos]; %ClosLS：节点中间中心性和其对应坐标的中心—坐标矩阵，第一列为位置向量，第二列为中间中心性的得分
        
        NodeD = B(ClosMinLoc(1), :); %NodeD：选取目标节点BetMinLoc(1)的关系向量
        [sU, eU] = find(NodeD == 1); %找到和最小中间中心性节点已经连接的节点个数sU和坐标向量eU
        ClosLC(eU, :) = []; %将已经和目标节点相连的节点从中心—坐标矩阵中排除
        BetLS_Sort = sortrows(ClosLC, 2); %BetLS_Sort：将中心—坐标矩阵按第二列的中间中心性进行排序
        
        B(ClosMinLoc(1), BetLS_Sort(1, 1)) = 1; B(BetLS_Sort(1, 1), ClosMinLoc(1)) = 1; %将最小中间中心性的节点BetMinLoc(1)和第二小的节点BetLS_Sort(1, 1)连接
        G1 = graph(B,'upper', 'omitselfloops');
